  ...
  return <div role="presentation" onMouseDown={onClose} style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,23,42,.42)", display: "grid", placeItems: "center", padding: 16 }}>
    <section role="dialog" aria-modal="true" aria-label="Connect communication to lead" onMouseDown={(event) => event.stopPropagation()} style={{ width: "min(620px, 100%)", maxHeight: "min(720px, 90vh)", overflow: "hidden", background: "#fff", borderRadius: 10, boxShadow: "0 20px 50px rgba(0,0,0,.25)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ overflowY: "auto", minHeight: 180 }}>{loading ? <p style={{ padding: 20 }}>Loading leads…</p> : selectedLead ? <div style={{ padding: 20, display: "grid", gap: 14 }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, flexWrap: "wrap" }}><button className="slds-button" type="button" onClick={() => setSelectedLead(null)} style={{ padding: "9px 14px", border: "1px solid #0176d3", borderRadius: 5, background: "#fff", color: "#0176d3", cursor: "pointer" }}>Back</button><button className="slds-button" type="button" disabled={Boolean(saving)} onClick={() => void connect(selectedLead)} style={{ padding: "9px 14px", border: 0, borderRadius: 5, background: "#0176d3", color: "#fff", cursor: saving ? "wait" : "pointer", fontWeight: 700 }}>{saving === selectedLead.id ? "Connecting…" : "Connect to this lead"}</button></div>
      </div> : items.length ? items.map((lead) => <button className="slds-button" key={lead.id} type="button" disabled={Boolean(saving)} onClick={() => setSelectedLead(lead)} style={{ width: "100%", border: 0, borderBottom: "1px solid #e2e8f0", background: "#fff", padding: "13px 20px", textAlign: "left", cursor: saving ? "wait" : "pointer" }}><strong style={{ color: "#0b5cab" }}>{lead.name || "Unnamed lead"}</strong><span style={{ display: "block", color: "#475569", fontSize: 12, marginTop: 4 }}>{[lead.company, lead.phone, lead.email].filter(Boolean).join(" · ")}</span></button>) : creating ? <div style={{ padding: 20, display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, flexWrap: "wrap" }}><button className="slds-button" type="button" onClick={() => setCreating(false)} style={{ padding: "9px 14px", border: "1px solid #0176d3", borderRadius: 5, background: "#fff", color: "#0176d3", cursor: "pointer" }}>Back</button><button className="slds-button" type="button" disabled={!fullName.trim() || !companyId || Boolean(saving)} onClick={() => void createLead()} style={{ padding: "9px 14px", border: 0, borderRadius: 5, background: "#0176d3", color: "#fff", cursor: saving ? "wait" : "pointer", opacity: !fullName.trim() || !companyId ? .55 : 1 }}>{saving === "new" ? "Creating…" : "Create and connect"}</button></div>
      </div> : <div style={{ padding: 20 }}><p style={{ margin: "0 0 14px", color: "#64748b" }}>No matching leads found.</p><button className="slds-button" type="button" onClick={() => { setFullName(search.trim()); setCreating(true); }} style={{ padding: "10px 15px", border: 0, borderRadius: 5, background: "#0176d3", color: "#fff", cursor: "pointer", fontWeight: 700 }}>Create new lead</button></div>}</div>
    </section>
  </div>;
}
