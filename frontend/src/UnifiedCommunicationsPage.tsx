...
type TimelineItem = { id: string; kind: "message" | "call"; channel: string; direction: "inbound" | "outbound"; timestamp: number; text: string; senderLabel?: string; answered?: boolean; reason?: string };
...
const messagePreview = (source: Record<string, unknown>, platform: string) => {
  ...
  return `${channel} · ${sender ? `${sender}: ` : ""}${String(source.message || "No preview")}`;
};
