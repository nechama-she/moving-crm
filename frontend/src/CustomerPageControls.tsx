...
export default function CustomerPageControls({leadId}:{leadId:string}){
 const {token,user}=useAuth();const [data,setData]=useState<Page>(),[price,setPrice]=useState(''),[cuft,setCuft]=useState(''),[company,setCompany]=useState(''),[companies,setCompanies]=useState<{id:string;name:string}[]>([]),[message,setMessage]=useState(''),[busy,setBusy]=useState(false);
 ...
 const hasPage=!!data;
 const load=useCallback(async()=>{const r=await fetch(base,{headers:authHeaders(token)});if(r.status===404||r.status===403)return;if(!r.ok)throw new Error('Could not load customer page');const d=await r.json();setData(d);setPrice(d.price);setCuft(d.cuft);setCompany(d.company_id);},[base,token]);
 ...
 {user?.role==='admin'&&<label style={{display:'block',marginTop:12}}>Company <select value={company} onChange={e=>setCompany(e.target.value)}><option value="">Not connected</option>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>}
 <div style={{display:'flex',gap:12,margin:'14px 0',flexWrap:'wrap'}}><label>Total cubic feet<input type="number" min="0.01" step="0.01" value={cuft} onChange={e=>setCuft(e.target.value)} style={{display:'block',width:130}}/></label><label>Price ($)<input type="number" min="0" step="0.01" value={price} onChange={e=>setPrice(e.target.value)} style={{display:'block',width:130}}/></label></div><button disabled={busy} onClick={()=>void save()}>Save draft</button>{' '}<button disabled={busy||!price||!cuft} onClick={()=>void save(true)}>Publish estimate</button><p style={{fontSize:12,color:'#64748b'}}>{data.published?'Customer sees the last published estimate.':'Customer sees: We’re preparing your estimate.'}</p>
