...
export default function CustomerMovePage() {
  ...
  return <div className="customer-move"><div className="cm-wrap">
    {!session?<section className="cm-verify"><div className="cm-eyebrow">WELCOME</div><h1>LetÃ¢â‚¬â„¢s get your<br/>move underway.</h1><p>First, verify itÃ¢â‚¬â„¢s you. WeÃ¢â‚¬â„¢ll send a code to the phone or email you provided.</p>{error&&<div role="alert" className="cm-error">{error}</div>}
      ...
      {!sent?<button className="cm-primary" disabled={busy||!channel||!key} onClick={()=>void send()}>{busy?'SendingÃ¢â‚¬Â¦':'Send verification code'}</button>:<form onSubmit={e=>{e.preventDefault();void verify();}}><label className="cm-code">Enter your 6-digit code<input inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,''))}/></label><button className="cm-primary" disabled={busy||code.length!==6}>{busy?'CheckingÃ¢â‚¬Â¦':'View my move'}</button><button type="button" className="cm-text-button" disabled={busy||wait>0} onClick={()=>void send()}>{wait?`Send again in ${wait}s`:'Send a new code'}</button></form>}<small className="cm-private">Your information is private. No account or password needed.</small></section>:
    !data?<p role="status">Opening your moveÃ¢â‚¬Â¦</p>:<><section className="cm-intro"><div><div className="cm-eyebrow">LETÃ¢â‚¬â„¢S MAKE YOUR NEXT MOVE EASIER</div><h1>Hi {data.name.split(' ')[0]},<br/>youÃ¢â‚¬â„¢re in the right place.</h1><p>Share a little more about your home.<br/>WeÃ¢â‚¬â„¢ll take care of the estimate.</p></div><div className="cm-estimate"><span>{data.estimate?'Your moving estimate':'Your estimate'}</span><strong>{data.estimate?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(Number(data.estimate.price)):'WeÃ¢â‚¬â„¢re working on it.'}</strong><p>{data.estimate?`${Number(data.estimate.cuft).toLocaleString()} cubic feet estimated`:'Add photos or request a video walkthrough to help us prepare your estimate.'}</p></div></section>
