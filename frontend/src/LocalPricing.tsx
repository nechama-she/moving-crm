...
type Quote = { travel_complete: boolean; travel_hours: number; travel_hourly_rate: number | null; recommended_crew: number; crew_size: number; trucks: number; capacity: number; estimated_hours: number; billable_hours: number; hourly_rate: number | null; full_pack_hourly: number; minimum_applied: boolean; total: number | null; warning: string; charges: { name: string; description: string; subtotal: number; discountAmount: number; totalCost: number }[] };
  ...
  return <div className="local-pricing">
    ...
    {!editing && settings && <section className="pricing-card pricing-calculator local-calculator">
      ...
      <section className="local-travel" aria-label="Office travel estimate">
        ...
        {travel && <><div className="local-travel-legs"><div><span>Office to pickup</span><strong>{number(travel.office_to_pickup_miles)} miles</strong></div><div><span>Delivery to office</span><strong>{number(travel.delivery_to_office_miles)} miles</strong></div><div><span>Total travel</span><strong>{number(travel.total_minutes)} min</strong></div></div><small className="local-hint">Approximate straight-line mileage, not road distance. {travel.uses_zip_centers ? "One or more locations use ZIP centers. " : ""}Road distance can be longer.</small></>}
        <p className="local-hint">Travel fee = combined miles / 60, rounded to the nearest whole hour, x {settings.travel_hourly_rate == null ? "the moving crew hourly rate" : `${money(settings.travel_hourly_rate)}/hour`}. Minimum 1 travel hour; half an hour rounds up. {settings.travel_in_minimum ? "Travel time counts toward the minimum hours." : "Travel is added on top of the moving minimum."} Packing is charged for moving hours only.</p>
      </section>
    ...
    <form className="pricing-card local-settings" onSubmit={saveSettings}>
      ...
      <fieldset disabled={saving}>
        ...
        <div className="local-setting-basics">
          <label>Travel hourly rate{editing ? <input aria-label="Travel hourly rate" type="number" min="0" step="0.01" value={draft.travel_hourly_rate ?? ""} placeholder="Use moving crew rate" onChange={e => setDraft(current => ({ ...current, travel_hourly_rate: numeric(e.target.value) }))} /> : <strong>{settings?.travel_hourly_rate == null ? "Moving crew rate" : money(settings.travel_hourly_rate)}</strong>}</label>
