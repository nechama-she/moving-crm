...
export default function CustomerExtraStopsQuestion({group,apiKey,onChange,onIncomplete,missing}:{group:ExtraStopGroup;apiKey:string;onChange:(answer:boolean,stops:string[])=>void;onIncomplete:(incomplete:boolean)=>void;missing:boolean}){
 ...
 {answer && <><p>Select each address from the Google Maps suggestions.</p>{rows.map((row,index)=>{const saved=group.stops.find(r=>r.address===row.address);return <div key={row.id} style={{border:'1px solid #e5d8d5',borderRadius:10,padding:14,marginBottom:12}}><CustomerAddressInput label={`Extra ${group.location} stop ${index+1}`} apiKey={apiKey} initialValue={row.address} disabled={false} error={missing&&!row.confirmed?'Select an address from the suggestions.':undefined} onChange={text=>{setRows(old=>old.map(r=>r.id===row.id?{...r,address:text,confirmed:false}:r));onIncomplete(true);}} onSelect={address=>select(row.id,address)}/>
 ...
 <button type="button" className="cm-secondary-btn" onClick={()=>{const next=rows.filter(r=>r.id!==row.id);setRows(next);onIncomplete(!next.length||next.some(r=>!r.confirmed));onChange(true,next.filter(r=>r.confirmed).map(r=>r.address));}}>Remove stop</button></div>;})}<button type="button" className="cm-secondary-btn" onClick={()=>{setRows(old=>[...old,{id:crypto.randomUUID(),address:'',confirmed:false}]);onIncomplete(true);}}>+ Add another {group.location} stop</button></>}
