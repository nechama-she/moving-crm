...
export default function ReportsPage() {
  ...
  return <main className="reports-page">
    ...
    <details className="reports-method"><summary>How this is counted</summary><p>A customer is matched by phone or email across companies. Matching move dates, pickups and deliveries count as one move, assigned to its earliest stored signup. Different move details count separately. A booking on any matching lead counts, even after the selected period.</p><p>Booked, scheduled, completed, or a recorded booking date count as booked. Missing move or contact details stay separate for review. Address edits, shared contact details, and ZIP-only locations can affect matching.</p><p>This report uses stored data only. Missing SmartMoving signup times use the CRM creation time and are flagged. Matching SmartMoving and CRM timestamps are also flagged. No external requests or data updates are performed.</p></details>
    ...
    {report && <>
      ...
      {(report.review_count > 0 || report.undated_moves > 0) && <p className="reports-warning">{report.review_count} moves in this range need data review. {report.undated_moves > 0 ? `${report.undated_moves} moves have no usable signup time and are excluded from date-based results.` : "Open the rows below to see details."}</p>}
