...
export default function LiveSwitchPanel({ leadId, onClose, onUploaded }: { leadId: string; onClose: () => void; onUploaded: () => void }) {
  ...
  return createPortal(<div className="ls-backdrop" onClick={() => { if (!busy) onClose(); }}>
    <div className="ls-panel" role="dialog" aria-modal="true" aria-labelledby="ls-title" tabIndex={-1} ref={panel} onClick={e => e.stopPropagation()} onKeyDown={e => {
      ...
      <header><span className="ls-logo"><Icon kind="video"/></span><div><h2 id="ls-title">LiveSwitch</h2><p>Connect and share files with your customer</p></div><button aria-label="Close LiveSwitch" disabled={busy} onClick={onClose}>×</button></header>
      <main>{loading ? <p role="status">Preparing your conversation…</p> : error ? <div role="alert" className="ls-error">{error}<button onClick={() => void load()}>Try again</button></div> : conversation ? <>
        ...
        <div className="ls-files">{items.map(item => <article key={item.id}>{item.preview ? <img src={item.preview} alt=""/> : <span className="ls-file-icon">{item.type.startsWith("video/") ? "?" : "PDF"}</span>}<div className="ls-file-content"><strong title={item.name}>{item.name}</strong><small>{(item.file.size / 1024 / 1024).toFixed(1)} MB · {item.status}</small><progress value={item.progress} max={100} aria-label={`${item.name} upload progress`}/>{item.error && <span className="ls-error" role="status">{item.error}</span>}</div>{!busy && !item.crm && !item.live && <button aria-label={`Remove ${item.name}`} onClick={() => setItems(current => current.filter(row => row.id !== item.id))}>×</button>}{item.crm && item.live && <span className="ls-success" aria-label="Uploaded">?</span>}</article>)}</div>
