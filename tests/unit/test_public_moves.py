...
def test_stairs_autosave_keeps_waived_line_and_only_updates_current_address(portal, packing_pricing, monkeypatch):
    from decimal import Decimal
    mod,db,lead,access=portal
    job=db.get(models.LeadJob,access.job_id)
    lead.volume=24
    ...
    plan=SimpleNamespace(services=[SimpleNamespace(comments='__stairs_pricing__:'+json.dumps({
        'enabled':True,'steps_per_flight':13,'free_flights':1,'rate_per_cuft':'.20','minimum_cubic_feet':286,'pickup_discount_percent':100}))],rates=[])
    packing_pricing.customer_stairs.__globals__['infer_job_move_type']=lambda *args:('Long Distance',plan)
    monkeypatch.setitem(sys.modules,'routes.leads',MagicMock())
    monkeypatch.setattr(mod,'_move_details',lambda *args,**kwargs:{'ok':True})
    db.commit()
    def save(location,flights,revision=None):
        question=next(r for r in packing_pricing.customer_stairs(lead,job,db)['locations'] if r['location']==location)
        mod.save_customer_packing(mod.CustomerPackingPatch(change=mod.CustomerPackingChange(kind='stairs',location=location,flights=flights,revision=revision or question['revision'])),access,db)
    save('pickup',2)
    pickup=db.query(models.LeadJobCharge).filter_by(name='Pickup stairs').one()
    assert (pickup.subtotal,pickup.discount_amount,pickup.total_cost)==(Decimal('57.20'),Decimal('57.20'),0)
    save('delivery',2)
    assert job.price==access.published_price==Decimal('1057.20')
    save('delivery',2)
    assert db.query(models.LeadJobCharge).count()==3
    assert job.price==Decimal('1057.20')
    save('delivery',0)
    assert job.price==access.published_price==1000
...
def test_long_carry_autosave_keeps_waived_line_and_only_updates_current_address(portal, packing_pricing, monkeypatch):
    from decimal import Decimal
    mod,db,lead,access=portal
    job=db.get(models.LeadJob,access.job_id)
    lead.volume=24
    ...
    job.customer_packing_package=json.dumps({'mode':'full'})
    db.add(models.LeadJobCharge(id='unchanged',job_id=job.id,name='Transportation',subtotal=1000,total_cost=1000))
    plan=SimpleNamespace(services=[SimpleNamespace(comments='__long_carry_pricing__:'+json.dumps({
        'enabled':True,'increment_feet':75,'included_feet':75,'rate_per_cuft':'.20','minimum_cubic_feet':286,'pickup_discount_percent':100}))],rates=[])
    packing_pricing.customer_long_carry.__globals__['infer_job_move_type']=lambda *args:('Long Distance',plan)
    monkeypatch.setitem(sys.modules,'routes.leads',MagicMock())
    monkeypatch.setattr(mod,'_move_details',lambda *args,**kwargs:{'ok':True})
    db.commit()
    def save(location,carry_feet,revision=None):
        question=next(r for r in packing_pricing.customer_long_carry(lead,job,db)['locations'] if r['location']==location)
        mod.save_customer_packing(mod.CustomerPackingPatch(change=mod.CustomerPackingChange(kind='long_carry',location=location,carry_feet=carry_feet,revision=revision or question['revision'])),access,db)
    save('pickup',100)
    pickup=db.query(models.LeadJobCharge).filter_by(name='Pickup long carry').one()
    assert (pickup.subtotal,pickup.discount_amount,pickup.total_cost)==(Decimal('57.20'),Decimal('57.20'),0)
    save('delivery',100)
    assert job.price==access.published_price==Decimal('1057.20')
    save('delivery',100)
