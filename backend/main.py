...
async def enforce_authentication(request: Request) -> None:
    ...
    if re.fullmatch(r"/api/public-moves/[0-9a-f-]{36}/(verify-options|send-code|verify|details|generate-inventory-report|walkthrough|reschedule|availability|files|prepare-upload|finish-upload|packing|item-answer|question-images|file-preview/[^/]+|inventory-catalog|manual-inventory|files/[^/]+|calculate-price|reports/[^/]+/select)", request.url.path):
